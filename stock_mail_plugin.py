...
def _run_llm_analysis(analysis):
    """阶段二：调用 DeepSeek 生成研判；失败则返回 None（回退规则报告）。"""
    try:
        import analyzer
        return analyzer.analyze(analysis, verify=False)
    except Exception as e:
        logger.error("LLM analysis failed, fallback to rule report: %s" % e)
        return None
...
def send_enhanced():
    ...
    if analysis and llm_result:
        report_html = generate_llm_report(analysis, llm_result)
        html_parts.append(report_html)
    elif analysis:
        report_html = generate_market_report(analysis)
        html_parts.append(report_html)
    else:
        html_parts.append("<div class=header><h1>化资金讯早报</h1><p>" + now.strftime("%Y年%m月%d日") + "</p></div>")
    ...
    html_parts.append("<div class=footer><p>化资金讯机器人自动发送</p></div>")
    ...
    try:
        ...
        msg["From"] = Header("化资金讯机器人 <" + email_sender.sender_email + ">", "utf-8")
    ...
    # Persist A股 snapshot for traceability (phase-1 + phase-2 LLM 结论)
    if analysis:
        try:
            snap_id = db.save_market_snapshot(analysis, llm_result)
            logger.info("Market snapshot saved, id=%s", snap_id)
        except Exception as e:
            logger.warning("Snapshot save failed: %s" % e)

    # Push to Feishu and WeChat (去 Codex，本模块直接编排)
    if analysis:
        try:
            result = send_market_notification(analysis, llm_result)
